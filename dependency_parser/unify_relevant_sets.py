...
def relevant():
    for group in ['control', 'patients']:
        for pos_tag_to_test in ['nouns', 'verbs']:
            root_dir = os.path.join(DATA_DIR, 'relevant_sets', group)
            json_pattern = os.path.join(root_dir, 'relevant_set_{}*.json'.format(pos_tag_to_test))
            ...
            for pos, tag_list in relevant_dict.items():
                # take 15 most occurring items
                most_occurrences = Counter(tag_list).most_common()[:15]
                tag_list = [item[0] for item in most_occurrences]
                relevant_set[pos] = tag_list

            save_location = os.path.join(root_dir, '{}_norm_relevant_set_{}.json'.format(group, pos_tag_to_test))
...
def reference():
    for pos_tag_to_test in ['nouns', 'verbs']:
        ...
        save_location = os.path.join(root_dir, 'norm_reference_set_{}.json'.format(pos_tag_to_test))
        with open(save_location, 'w', encoding='utf-8') as out_file:
