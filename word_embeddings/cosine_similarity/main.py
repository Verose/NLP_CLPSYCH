...
def cosine_similarity_several_window_sizes(window_sizes):
    ...
    for i, win_size in tqdm(enumerate(window_sizes), file=sys.stdout, total=len(window_sizes), leave=False,
        ...
        cosine_calcs = POSSlidingWindow(model, data, win_size, DATA_DIR, pos_tags, conf['window_method'])
        cosine_calcs.calculate_all_scores()

        win_test = WindowTTest(pos_tags, win_size, [])
        ...
        if conf['output']['plot']:
            ...
            control_scores = cosine_calcs.get_scores('control')
            patients_scores = cosine_calcs.get_scores('patients')
            plot_groups_histograms(control_scores, patients_scores, win_size, pos_tags, OUTPUTS_DIR)
